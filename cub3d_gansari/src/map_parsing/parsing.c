...
/*   Updated: 2025/08/14 17:37:00 by mukibrok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_texture(char *line, t_game *map)
{
	if (ft_strncmp(line, "NO ", 3) == 0)
		map->textures.north.path = ft_strdup(line + space_count(line));
	if (ft_strncmp(line, "SO ", 3) == 0)
		map->textures.south.path = ft_strdup(line + space_count(line));
	if (ft_strncmp(line, "WE ", 3) == 0)
		map->textures.west.path = ft_strdup(line + space_count(line));
	if (ft_strncmp(line, "EA ", 3) == 0)
		map->textures.east.path = ft_strdup(line + space_count(line));
	else
		return (EXIT_FAILURE);
	return (EXIT_SUCCESS);
}
...
int	parse_file(t_game *game, char *filename)
{
	...
	while ((line = get_next_line(fd)))
	{
		...
		{
			map_started = 1;
			if (parse_map_line(line, &game->map, fd) == EXIT_FAILURE)
				return (print_error(ERR_INVALID), free(line), EXIT_FAILURE);
		}
		parse_texture(line, game);
		if (line[0] == 'F')
			game->map.floor_color = parse_color(line);
		if (line[0] == 'C')
			game->map.ceiling_color = parse_color(line);
		free(line);
	}
