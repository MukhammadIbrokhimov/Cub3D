...
/*   Updated: 2025/08/14 17:31:19 by mukibrok         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	validate_player(t_game *game)
{
	...
	while (++i < game->map.height && game->map.grid[i])
	{
		...
		{
			if (game->map.grid[i][j] == NORTH || game->map.grid[i][j] == SOUTH ||
				game->map.grid[i][j] == EAST || game->map.grid[i][j] == WEST)
			{
				player_count++;
				if (player_count > 1)
				...
				game->player.initial_dir = game->map.grid[i][j];
			}
			j++;
		}
	}
...
bool	validate_map_walls(t_game *game)
{
	...
	if (!temp_map)
		return (false);
	result = flood_fill(temp_map, (int)game->player.pos_y, (int)game->player.pos_x, &game->map);
